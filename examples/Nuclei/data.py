...
def get_train_dataflow():
    ...
    imgs = COCODetection.load_many(
        config.BASEDIR, config.TRAIN_DATASET, mode='train')
    """
...
#    imgs = list(filter(lambda img: len(img['boxes']) > 0, imgs))    # log invalid training
    ...
    # note that some transforms should be applied to image and masks in the same way
    # may consider random crop, color jitter etc, see discussions in kaggle about clustering
    # src: common, misc
    aug = imgaug.AugmentorList(
        [CustomResize(config.SHORT_EDGE_SIZE, config.MAX_SIZE),
         imgaug.Flip(horiz=True, vert=True)])

    def preprocess(img):
        fname = img['file_name']
        im = cv2.imread(fname, cv2.IMREAD_COLOR)   # cv2.IMREAD_UNCHANGED
        if im.shape[2] != 3: # just in case
            im = im[:,:,:3]
        assert im is not None, fname
        im = im.astype('float32')
        
        # load masks
        mask_names = os.listdir(img['mask_dir'])
        mask_paths = [img['mask_dir'] + mask_name for mask_name in mask_names]
        ...
        # augmentation for images and masks here
        ...
        # create boxes etc
        img['height'], img['width'], _ = im.shape
        k = len(masks)
        img['boxes'] = extract_bboxes(mask)
        img['class'] = np.ones(k) # hard coded here, (k,)
        img['is_crowd'] = np.ones(k)==0 # Use k False if you don't know what it means
        
        boxes, klass, is_crowd = img['boxes'], img['class'], img['is_crowd']
        # rpn anchor:
        try:
...
def get_eval_dataflow():
    imgs = COCODetection.load_many(config.BASEDIR, config.VAL_DATASET, mode='val')
